                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                ...
                                html.Div(
                                    id="compare-action-container",
                                    className="control-box",
                                    style={"display": "none"},
                                    children=[
                                        html.Label("Compare Mode Action"),
                                        dcc.RadioItems(
                                            id="compare-action",
                                            options=[
                                                {
                                                    "label": " Compare existing images in clusters",
                                                    "value": "existing",
                                                },
                                                {
                                                    "label": " Generate new images",
                                                    "value": "generate",
                                                },
                                            ],
                                            value="existing",
...
@app.callback(
    ...
    Output("compare-umap-plot", "figure"),
    Input("selected-point-store", "data"),
    Input("generated-neighbor-store", "data"),
    Input("compare-action", "value"),
)
def update_compare_plots(selected_ids, generated_neighbor_ids, compare_action):
    if selected_ids is None:
        selected_ids = []

    if generated_neighbor_ids is None:
        generated_neighbor_ids = []

    if compare_action == "generate":
        highlight_ids = generated_neighbor_ids
    else:
        highlight_ids = selected_ids

    return (
        make_compare_scatter("PC1", "PC2", "PCA Projection", highlight_ids),
        make_compare_scatter("TSNE1", "TSNE2", "t-SNE Projection", highlight_ids),
        make_compare_scatter("UMAP1", "UMAP2", "UMAP Projection", highlight_ids),
    )
...
@app.callback(
    Output("generated-ct-store", "data"),
    Output("generated-neighbor-store", "data"),
...
def generate_ct_from_clicked_or_selected_space(
    ...
    if plot_mode != "compare_all" or compare_action != "generate":
    ...
    try:
        ...
        info = f"""
Generated from {projection_name} using {action_text}
Display method: {generation_method}
